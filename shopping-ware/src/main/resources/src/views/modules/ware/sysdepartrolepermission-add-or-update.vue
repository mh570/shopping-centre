    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/ware/sysdepartrolepermission/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.departId = data.sysDepartRolePermission.departId
                this.dataForm.roleId = data.sysDepartRolePermission.roleId
                this.dataForm.permissionId = data.sysDepartRolePermission.permissionId
                this.dataForm.dataRuleIds = data.sysDepartRolePermission.dataRuleIds
                this.dataForm.operateDate = data.sysDepartRolePermission.operateDate
                this.dataForm.operateIp = data.sysDepartRolePermission.operateIp
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/ware/sysdepartrolepermission/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
